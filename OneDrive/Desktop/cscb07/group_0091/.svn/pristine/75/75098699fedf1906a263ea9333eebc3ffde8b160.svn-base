 ...
 * It also supports redirection into a file(appending or overwrite)
 * 
 * @author Sankalp Sharma
 **/
...
public class Pwd extends Command {
  ...
  /**
   * The run method executes if correct arguments are entered. It gets the 
   * current path that the User is in. It will throw an error for any arguments 
   * after pwd or redirection errors such as bad file name or 
   * invalid arguments for redirection.
   * 
   * @param args Arguments for the command
   * @return String output of the pwd command (current working directory), otherwise
   * nothing if redirection happened correctly 
   * 
  ...
  NodeExistsException,
    ...
    if (args.length == 3 && (args[1].equalsIgnoreCase(">") || args[1].
        equalsIgnoreCase(">>"))) {
  ...
  RedirectionException {
    ...
    if (args.length != 1 && args.length != 3) {
      ...
      // if doing 3 args, make sure redirection is done correctly
        ...
        && (!(args[1].equalsIgnoreCase(">") || args[1].
            equalsIgnoreCase(">>")))) {
