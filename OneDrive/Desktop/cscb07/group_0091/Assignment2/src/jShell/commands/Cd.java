...
/**
 * The Cd command is used for changing directories. It will throw an error if the  
 * directory doesn't exist or path is invalid or if you try to change to 
 * more than one directory at same time
 * 
 * @author Sankalp Sharma
 * 
 */
public class Cd extends Command {
  ...
  /**
   * The run method for CD uses the getNodeAtPath method from the FileSystemclass 
   * to determine the right course of action when changing directories such as: 
   * going up one level, staying at current level, and changing directories with
  ...
  public String run(String[] args)
    ...
    try {
      dirDestination =
          (IDirectory) IShell.getShell().getFileSystem().getNodeAtPath(new
              Path(args[1]));
     ...
     * then we just want to manually set it to root path
     */
